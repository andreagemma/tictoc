...
class TestTicTocTime:
    ...
    def test_string_parsing_and_time_math(self) -> None:
        ...
        assert int(later) - int(instant) == 10
        assert int(later) - int(datetime.fromtimestamp(float(instant)).timestamp()) == 10
        shifted = cast(TicTocTime, later - timedelta(seconds=5))
        assert isinstance(shifted, TicTocTime)
        assert int(shifted) - int(instant) == 5
